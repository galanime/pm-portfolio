...
将仅首屏的点线背景替换为固定视口的蓝白光场：缓慢流动的光带、漂浮光点、指针柔光、点击涟漪和滚动相位变化。装饰层 pointer-events:none，置于 body 隔离层的负层级，不改变人像与项目内容的布局。移动端降低粒子数、画布分辨率和帧率；后台页面及系统减少动效偏好会停止动画，不添加页面控制按钮。
